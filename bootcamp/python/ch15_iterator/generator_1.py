# 제너레이터(Generator)는 이터레이터(Iterator)를 만드는 가장 쉽고 간편한 방법입니다. 
# 모든 제너레이터는 이터레이터이지만, 모든 이터레이터가 제너레이터인 것은 아닙니다. 
# 1. 구현 방식의 차이
# 이터레이터: 클래스 내부에 __iter__와 __next__ 메서드를 직접 정의해야 합니다. 
# 질문자님이 위에서 작성하신 코드가 전형적인 이터레이터 방식입니다.
# 제너레이터: 일반 함수처럼 작성하되, return 대신 yield 키워드를 사용합니다. 
# 파이썬이 자동으로 __iter__와 __next__를 구현해주므로 코드가 훨씬 간결합니다. 

# 2. 상태 관리 (State Management)
# 이터레이터: self.position과 같이 현재 어디까지 진행됐는지 상태를 변수로 직접 저장하고 관리해야 합니다.
# 제너레이터: 함수가 실행되다가 yield를 만나면 그 지점의 상태(로컬 변수 등)를 자동으로 기억하고 멈춥니다. 
# 다음 호출 시 멈췄던 지점부터 다시 시작합니다. 

# 3. 메모리 효율성
# 둘 다 데이터를 한꺼번에 메모리에 올리지 않고 필요할 때마다 하나씩 생성하는 
# 게으른 연산(Lazy evaluation)을 수행하여 메모리를 절약합니다.
...
squares = (x**2 for x in range(10))  # 메모리에 값 없음
...
# print("방법3: map, lmabda, range를 이용")
# result = map(lambda x: x ** 2, range(1, 6))
# for num in result:
#     print(num)

# print("방법4: iterator 클래스 - 모든 제너레이터는 이터레이터이다")
...
class FoodIterator:
    ...
    def __next__(self):
        if self.index >= len(self.data):
            raise StopIteration  # 끝나면 순회 종료
        item = self.data[self.index]
        self.index += 1
        return item

it = FoodIterator(food)
for item in it:
    print(item)  # 김밥, 만두, 양념치킨, 족발, 피자, 쫄면, 라면

# 방법 3. 제너레이터 함수 (yield)

def food_generator(data):
    for item in data:
        yield item  # 하나씩 꺼내 반환 후 일시 중단

gen = food_generator(food)
print(next(gen))  # 김밥
print(next(gen))  # 만두
for item in gen:
    print(item)  # 양념치킨, 족발, 피자, 쫄면, 라면

# 방법 4. 제너레이터 표현식

gen_expr = (item for item in food)  # () 사용 → 즉시 계산 안 함

for item in gen_expr:
    print(item)  # 김밥, 만두, 양념치킨, 족발, 피자, 쫄면, 라면


# 파일에서 한 줄씩 읽는 제너레이터를 생성하고 사용하시오.
